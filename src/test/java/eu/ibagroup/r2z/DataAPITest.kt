...
class DataAPITest {
  ...
  private val dataAPI = buildApi<DataAPI>(zosmfUrl, UnsafeOkHttpClient.getUnsafeOkHttpClient())
  private val infoAPI = buildApi<InfoAPI>(zosmfUrl, UnsafeOkHttpClient.getUnsafeOkHttpClient())
  ...
  fun testListDatasets() {
    ...
    response.body()?.items?.forEach {
      print(it.name)
      println(it.datasetOrganization)
      println(it.migrated?.value)
    }
  ...
  fun testListDatasetMembers() {
    val request = dataAPI.listDatasetMembers(
      authorizationToken = basicCreds,
      datasetName = "KIRYL.IJMP.SAMPLIB"
    )
    val response = request.execute()
    assert(request.isExecuted)
    assert(response.code() == 200)
  ...
  fun testDeleteUssFile() {
    val request = dataAPI.deleteUssFile(
      authorizationToken = basicCreds,
      filePath = "u/KIRYL/ijmp/readme5.md",
    )
    val response = request.execute()
    assert(response.isSuccessful)
  }
